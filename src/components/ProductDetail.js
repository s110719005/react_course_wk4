...
function ProductDetail({ product }) {
   ...
   const [color, setColor] = useState(product.color > 0 ? product.color : "none");
   const handleChange = (value) => {
      setQty(value);
      
    };
   const handleChange2 = (colorVal) => {
               ...
               <p className="product-qty">
                  ...
                  >
                     {[...Array(product.color.length).keys()].map((x) => (
                        <Option key={x +1} colorVal={product.color[x]}>
                           {product.color[x]}
                        </Option>
                     ))}
                  </Select>
